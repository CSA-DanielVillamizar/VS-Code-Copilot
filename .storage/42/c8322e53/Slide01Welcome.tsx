...
const Slide01Welcome: React.FC = () => {
  ...
  return (
    <section 
      className="relative flex h-full flex-col items-center justify-start text-center px-12 py-8 overflow-y-auto"
      style={{
    ...
    >
      ...
      <div className="relative z-10 max-w-5xl w-full">
        <h1 
          ref={titleRef}
          className="text-5xl font-bold tracking-tight text-white mb-4 mt-4"
        >
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-5 mb-6 max-w-3xl mx-auto border border-slate-700">
        ...
        {/* Agenda */}
        <div ref={agendaRef} className="mb-6">
          <h2 className="text-2xl font-semibold text-white mb-6">Agenda</h2>
          <div className="grid grid-cols-2 gap-4 max-w-4xl mx-auto">
            {agendaItems.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-center gap-3 bg-slate-800/50 backdrop-blur-sm px-5 py-3 rounded-xl border border-slate-700 hover:border-blue-500 transition-colors"
                >
                  <Icon className="w-5 h-5 text-blue-400 flex-shrink-0" />
                  <span className="text-base text-slate-200 text-left">{item.text}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-3 max-w-3xl mx-auto">
