...
import { Copy, Check, Rocket, Code2, Download } from "lucide-react";
...
IMPORTANTE: El código debe ser completamente funcional desde el primer momento. Incluye comentarios explicativos en español. La aplicación debe verses profesional y ser intuitiva de usar.`;

  const handleCopy = () => {
    try {
      // Create a temporary textarea element
      const textarea = document.createElement('textarea');
      textarea.value = demoPrompt;
      ...
      if (successful) {
        ...
        navigator.clipboard.writeText(demoPrompt).then(() => {
          setCopied(true);
          setTimeout(() => setCopied(false), 2000);
        }).catch(err => {
  ...
  const handleDownload = () => {
    const blob = new Blob([demoPrompt], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'prompt-lista-ideas-demo.txt';
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="flex flex-col">
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm p-3 rounded-xl border border-slate-700 mb-3">
              <p className="text-xs text-slate-300">
                <strong className="text-blue-300">Instrucciones:</strong> Usa el botón "Copiar" para copiar al portapapeles, o "Descargar" para guardar como archivo .txt
              </p>
            </div>
            ...
            <div className="mt-3 space-y-2">
              <div className="bg-violet-500/10 border border-violet-500/30 rounded-lg p-3">
                <p className="text-xs text-violet-200">
                  <strong>💡 Tip Pro:</strong> Si el botón "Copiar" no funciona, usa "Descargar" para guardar el prompt como archivo de texto.
                </p>
              </div>
